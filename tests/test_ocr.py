...
async def test_process_pdf_to_markdown_delimited(tmp_path):
    ...
    with patch.object(
        OcrProcessor,
        "process_page",
        new=AsyncMock(side_effect=["# Page 1", "# Page 2"]),
    ):
        processor = OcrProcessor("http://fake", "fake", "fake", 10)
        md = await processor.process_pdf_to_markdown(
            str(pdf_path), delimiter="delimited"
        )
        assert md == "# Page 1\n\n---\n\n# Page 2"
...
async def test_process_pdf_to_markdown_error(tmp_path):
    ...
    with patch.object(
        ...
        "process_page",
        new=AsyncMock(
            return_value="**[ERROR: LM Studio API returned None for page 1]**"
        ),
    ):
        processor = OcrProcessor("http://fake", "fake", "fake", 10)
        md = await processor.process_pdf_to_markdown(
    ...
    with patch.object(
        ...
        "process_page",
        new=AsyncMock(
            return_value="**[ERROR: LM Studio API response missing 'choices' for page 1]**"
        ),
    ):
        processor = OcrProcessor("http://fake", "fake", "fake", 10)
        md = await processor.process_pdf_to_markdown(
    ...
    with patch.object(
        ...
        "process_page",
        new=AsyncMock(
            return_value="**[ERROR: LM Studio API response missing 'message.content' for page 1]**"
        ),
    ):
        processor = OcrProcessor("http://fake", "fake", "fake", 10)
        md = await processor.process_pdf_to_markdown(
        ...
        assert (
            "[ERROR: LM Studio API response missing 'message.content' for page 1]" in md
        )
...
async def test_process_page_success():
    ...
    with (
        patch("olmocr.pipeline.build_page_query") as mock_build_query,
        patch.object(
...
async def test_process_page_no_message_content():
    ...
    with (
        ...
        result = await processor.process_page("/fake/path.pdf", 1)
        assert (
            "ERROR: LM Studio API response missing 'message.content' for page 1"
            in result
        )
...
async def test_process_page_diagram_classification():
    ...
    mock_response.choices = [MagicMock()]
    mock_response.choices[0].message.content = json.dumps(
        {"natural_text": None, "is_diagram": True, "primary_language": "en"}
    )
...
async def test_process_page_table_classification():
    ...
    mock_response.choices = [MagicMock()]
    mock_response.choices[0].message.content = json.dumps(
        {"natural_text": None, "is_table": True, "is_diagram": False}
    )
...
async def test_process_page_missing_natural_text():
    ...
    mock_response = MagicMock()
    mock_response.choices = [MagicMock()]
    mock_response.choices[0].message.content = json.dumps({"some_other_field": "value"})
    ...
    with (
        ...
        result = await processor.process_page("/fake/path.pdf", 1)
        assert (
            "ERROR: LM Studio API response JSON missing 'natural_text' for page 1"
            in result
        )
...
async def test_process_page_general_exception():
    ...
    with patch(
        "olmocr.pipeline.build_page_query", side_effect=Exception("General error")
    ):
        result = await processor.process_page("/fake/path.pdf", 1)
        assert (
            "ERROR: Exception during OCR page 1" in result and "General error" in result
        )
