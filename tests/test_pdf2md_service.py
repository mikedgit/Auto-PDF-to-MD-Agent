...
from src.pdf2md_service import wait_for_file_stable, on_new_pdf, main
...
def test_on_new_pdf_file_deleted_before_processing(service_env):
    ...
    with patch('src.pdf2md_service.wait_for_file_stable', return_value=True), \
...
def test_on_new_pdf_success(service_env):
    ...
    with patch('src.pdf2md_service.wait_for_file_stable', return_value=True), \
         patch('src.pdf2md_service.ocr_pdf_to_markdown_sync', return_value="# Test Markdown"):
        on_new_pdf(str(pdf_path))
...
def test_on_new_pdf_processing_exception(service_env):
    ...
    with patch('src.pdf2md_service.wait_for_file_stable', return_value=True), \
         patch('src.pdf2md_service.ocr_pdf_to_markdown_sync', side_effect=Exception("Processing error")):
        on_new_pdf(str(pdf_path))
...
def test_main_normal_run(service_env):
    ...
    with patch('sys.argv', ['pdf2md_service.py']), \
